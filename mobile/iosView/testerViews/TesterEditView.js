...
class TesterEditView extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
        };
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.testerDeleteRequest) {
            if(nextProps.testerDeleteResponse) {
                 this.onDismissforDelete();
            } else if(nextProps.testerDeleteFailed) {
               AlertIOS.alert(null,nextProps.testerDeleteFailed,this.onDismiss.bind(this));
            }
        } else if(nextProps.testerEditRequest) {
          if(nextProps.testerEditResponse) {
               this.onDismiss();
          } else if(nextProps.testerEditFailed) {
             AlertIOS.alert(null,nextProps.testerEditFailed,this.onDismiss.bind(this));
          }
        }
    }
    ...
    onDismissforDelete() {
      Actions.pop();
      this.props.testerActions.resetSelecteTester(this.props.testerSelected.id);
    }


    onEditTester() {
      this.props.testerActions.updateTester(this.props.testerSelected.id, this.state.email, true,
      this.state.firstName, this.state.lastName, this.state.password, this.state.email);
    }

    deleteTester() {
        if(config.NETWORK_STATUS) {
          this.props.testerActions.deleteTester(this.props.testerSelected.id);
        } else {
    ...
    showConfirmationAlert() {
  		AlertIOS.alert(
  					 null,
  					 "Are you sure you want to delete tester?",
  					 [
  						 {text: 'Yes', onPress: () => this.deleteTester()},
  						 {text: 'No'},
  					 ]
  				 )
  	}
                    ...
                    <View style={styles.loginBox}>
                        <View style={{flex:1}}>
                                ...
                                <TextInput
                                   ...
                                   <TextInput
                                      ref='email'
                                      style={[AppStyles.textFieldStyle, {color:"#1F7EF6",marginTop: 2}]}
                                ...
                                <TextInput
                                  ...
                                  <TouchableOpacity style = {{flex:1, flexDirection:'row',
                                    backgroundColor:'#FF2600',height: 44,marginTop: 20,
                                     borderRadius: 3,
...
const mapStateToProps = (state) => ({
    ...
    "testerEditRequest":state.tester.testerEditRequest,
    "testerEditResponse":state.tester.testerEditResponse,
    "testerEditFailed":state.tester.testerEditFailed,
});
...
export default connect(mapStateToProps, mapDispatchToProps)(TesterEditView);
